...
export default class EmailManagement {
  static run() {
    this.checkTime();
  }
  ...
  static checkTime() {
    ...
    if (now.hour() == 3) {
      ...
      switch (now.day()) {
        case 0:
          //Sunday
          this.sendReports(ReportsEnum.frequency[6].value);
          break;
        case 1:
          //Monday
          this.sendReports(ReportsEnum.frequency[0].value);
          break;
        case 2:
          //Tuesday
          this.sendReports(ReportsEnum.frequency[1].value);
          break;
        case 3:
          //Wednesday
          this.sendReports(ReportsEnum.frequency[2].value);
          break;
        case 4:
          //Thursday
          this.sendReports(ReportsEnum.frequency[3].value);
          break;
        case 5:
          //Friday
          this.sendReports(ReportsEnum.frequency[4].value);
          break;
        case 6:
          //Saturday
          this.sendReports(ReportsEnum.frequency[5].value);
          break;
      }

      //Check the day of Month
      if (now.date() === 1) {
        this.sendReports(ReportsEnum.frequency[7].value);
      }
      ...
      if ([28, 29, 30, 31].includes(now.date())) {
        ...
        if (tomorrow.date() === 1) {
          this.sendReports(ReportsEnum.frequency[8].value);
        }
      }
  ...
  static executeSchedule(schedule) {
    ...
    this.createReportPdf(userIds, clientIds, accounts, report);
  }
  ...
  static createReportPdf(userIds, clientIds, accounts, report) {
    ...
    pdf.create(ReportTable).toStream(
      Meteor.bindEnvironment((err, attachment) => {
        if (!err) {
          this.sendEmails(userIds, clientIds, attachment, report);
        }
      })
    );
  }
