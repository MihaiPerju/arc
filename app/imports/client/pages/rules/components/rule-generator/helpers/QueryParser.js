...
export default class QueryParser {
  ...
  static parseToData (query, config) {
    const data = null;
    const tokens = this.getTokensArray (
      query,
      config.combinators,
      config.operators
    );
    const asTree = ASTree.buildTree (tokens, config.combinators);
    return this.convertSyntaxTreeToData (
      asTree,
      data,
      config.combinators,
      '1',
  ...
  static convertSyntaxTreeToData (
    element,
    data,
    combinators,
    nodeName,
    ...
    const newCombinator = {
      combinator: firstCombinator
        ? firstCombinator.value
        : combinators[0].combinator,
      nodeName,
      rules: [],
    };
    ...
    for (let i = 0; i < element.children.length; i += 1) {
      this.convertSyntaxTreeToData (
        element.children[i],
        data,
        combinators,
        `${newCombName}/${currElement.rules.length + 1}`,
        newCombName
      );
    }
    return data;
  ...
  static getCombinatorsIndexes (query, combinators) {
    ...
    const combinatorsPattern = this.getSearchPattern (
      combinators,
      'combinator'
    );
    let match;
    while ((match = combinatorsPattern.exec (query)) !== null) {
      combinatorsIndexes.push ({
        start: match.index,
        end: combinatorsPattern.lastIndex,
      });
    }
  ...
  static getFirstCombinator (element, combinators) {
    let foundCombinator = element.children.find (x =>
      combinators.find (y => y.combinator === x.value)
    );
    if (!foundCombinator) {
      for (let i = 0; i < element.children.length; i += 1) {
        foundCombinator = this.getFirstCombinator (
          element.children[i],
          combinators
        );
      }
