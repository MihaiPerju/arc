...
export default class AgedAccounts extends React.Component {
  ...
  getAgedAccounts(filters) {
    ...
    setTimeout(() => {
      Meteor.call("agedAccounts.get", filters.selectedClientId, filters.selectedFacilityId, filters.selectedUserId, filterCondition, (err, responseData) => {
        if (!err) {
  ...
  getAgedAccountsChartData(filters) {
    ...
    let filterCondition = FilterService.getQuery(filters.selectedDateRange, filters.startDate, filters.endDate);
    setTimeout(() => {
      Meteor.call("agedAccounts.getPerHour", filters.selectedClientId, filters.selectedFacilityId, filters.selectedUserId, filterCondition, (err, chartData) => {
        if (!err) {
