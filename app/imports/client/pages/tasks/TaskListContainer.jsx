...
import MetaDataSlider from "/imports/client/pages/tasks/components/TaskContent/MetaData";
...
class TaskListContainer extends Pager {
  ...
  incrementViewCount = _id => {
    Meteor.call("tasks.increment_view_count", _id, err => {
      if (err) {
  ...
  getTask(currentTask) {
    ...
    for (task of data) {
      if (task._id == currentTask) return task;
    }
    return null;
  }
  ...
  getTasks(tasksSelected) {
    ...
    let tasks = [];
    for (task of data) {
      if (tasksSelected.includes(task._id)) tasks.push(task);
    }
    return tasks;
  }
