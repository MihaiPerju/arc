...
Meteor.methods({
  ...
  "tasks.increment_view_count"(_id) {
    Tasks.update({ _id }, { $inc: { numberOfViews: 1 } });
  },
  ...
  "accounts.getSample"(filters) {
    const TasksRaw = Tasks.rawCollection();
    TasksRaw.aggregateSync = Meteor.wrapAsync(TasksRaw.aggregate);

    return TasksRaw.aggregateSync([
      { $match: filters },
      { $sample: { size: 20 } }
    ]);
  },
